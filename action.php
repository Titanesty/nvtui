...
$balance = $row['balance'];
  ...
  $win = $size * $colorWheel;
  $query = $mysqli->query("UPDATE kot_user SET balance = '$newbalance' WHERE hash = '$sid'");
...
if(!$_SESSION['hash']) {
    ...
    if($lay == 1) {
        if($rnd <= 10) {
            ...
            $fsum = $sum * 2;
            $update_sql2 = $mysqli->query("UPDATE kot_user SET balance = '$newbalance' WHERE hash = '$sid'");
...
$publickey = 'qoz5cyat8bn10rg'; // публичный ключ в настройках кассы
...
if($type == "continue_reg") {
  ...
  $dlpass = strlen($pass);
  $sql_select1 = sprintf("SELECT COUNT(*) FROM kot_user WHERE login='%s'", $mysqli->real_escape_string($login));
...
if($type == "registration") {
  ...
  $dlpass = strlen($pass);
  $sql_select1 = sprintf("SELECT COUNT(*) FROM kot_user WHERE login='%s'", $mysqli->real_escape_string($login1));
$result1 = $mysqli->query($sql_select1);
$row = mysqli_fetch_array($result1);
if($row)
{
...
$sql_select11 = sprintf("SELECT COUNT(*) FROM kot_user WHERE ip='%s'", $mysqli->real_escape_string($ip_c));
...
if($type == "login") {
  $login = $_POST['login'];
  $pass = $_POST['pass'];
  $sql_select1 = sprintf("SELECT COUNT(*) FROM kot_user WHERE login='%s' AND pass='%s'", $mysqli->real_escape_string($login), $mysqli->real_escape_string($pass));
$result1 = $mysqli->query($sql_select1);
$row = mysqli_fetch_array($result1);
if($row)
{
$count = $row['COUNT(*)'];
  ...
  if($error == 0) {
    $sql_select2 = sprintf("SELECT * FROM kot_user WHERE login='%s' AND pass='%s'", $mysqli->real_escape_string($login), $mysqli->real_escape_string($pass));
...
$result3 = $mysqli->query($sql_select3);
...
$result23 = $mysqli->query($sql_select23);
...
$sql_select4 = sprintf("SELECT COUNT(*) FROM kot_promo WHERE name='%s'", $mysqli->real_escape_string($name));
$result4 = $mysqli->query($sql_select4);
...
$sql_select = sprintf("SELECT COUNT(*) FROM kot_promo WHERE name='%s'", $mysqli->real_escape_string($promo));
$result = $mysqli->query($sql_select);
$row = mysqli_fetch_array($result);
if($row)
{	
...
$update_sql = "Update kot_user set balance='$balancenew' WHERE hash='$sid'";
      $mysqli->query($update_sql) or die("Ошибка вставки" . mysql_error());
$update_sql1 = "Update kot_user set bdate='$datab' WHERE hash='$sid'";
      $mysqli->query($update_sql1) or die("Ошибка вставки" . mysql_error());
$fa = "success";
...
$result27 = $mysqli->query($sql_select27);
...
$result0 = $mysqli->query($sql_select0);
